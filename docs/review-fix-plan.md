...
- [ ] **P1.1 — JSON whitespace mismatch → silent data loss.**
...
- [ ] **P1.2 — Partial raw-line copy expands tabs to spaces** (breaks DESIGN §4.3 /
...
- [ ] **P1.3 — `colorToHex` reads premultiplied RGBA, drops alpha.** `icons.go`. The
...
- [ ] **P1.4 — Search bar `Enter` skips match #1** within the debounce window.
...
- [ ] **P1.5 — `lineAndSubRow` returns a negative sub-row when `total()==0`.**
...
- [ ] **P1.6 — Source > 4 GiB silently wraps uint32 offsets.** `internal/parse`
...
- [ ] **P2.1 — XML `Detect` unbounded `bytes.Contains`** (~20-40× the bounded HTML
...
- [ ] **P2.2 — HTML structural-tag sniff misclassifies legitimate XML as HTML**
...
- [ ] **P2.3 — `WrapBreaks` recomputed up to 3× per visible line per reflow** (only
...
- [ ] **P2.4 — `indexMatches` reallocates its map every scan.** `search.go`. Fix:
...
- [ ] **P2.5 — Regex search `FindAllIndex` allocates per matching line + the regex
  path is unbenchmarked.** `search.go`. Fix: from-offset `FindIndex` loop mirroring
  `scanPlain`; add `BenchmarkSearchRegex`.
- [ ] **P2.6 — JSON literals/numbers lack a delimiter check** → trailing garbage
...
- [ ] **P2.7 — `CellOrigin`/`wrapRowSpan` allocate a fresh `WrapBreaks` per
  hit-test/search-nav under wrap.** `internal/geometry/geometry.go`. Fix: thread a
  reusable scratch `[]int32`.
