...
> **INVARIANT M-3 (model size).** The parsed document is a struct-of-arrays of flat arenas: one shared `[]byte` source (zero-copy segments), one `[]Node` (32 B/node, no pointers), one `[]Line` (24 B), one `[]Segment` (12 B), plus the fold index. Target ≈ 5× source bytes for typical pretty-printed JSON; documents dominated by short structural lines run higher, since each line/segment record is fixed-size. The 467 KiB `openapi.json` ⇒ ≈ 2.3 MB model (≈4.85×, the measured ratio guarded by `TestModelSizeRatio`); the 7.5 MB `big.json` ⇒ ≈ 51 MB (≈7.1×). No per-node heap allocation; no per-token `color.Color`; no per-line `[]rune` stored for the whole document.
...
├── icons.go                     // embedded Font Awesome Free toolbar glyphs (icons/fontawesome/*.svg, CC BY 4.0), themed resources colored at draw time
│
├── internal/geometry/
...
// (top level = depth 0; d <= 0 = never auto-collapse). Applies to subsequent
// SetData calls.
...
3. **Horizontal cull (MANDATORY, M-2):** the column window is the sub-row's `[startCol,endCol)` under soft-wrap (drawn from the left edge, `colBase = startCol`), else `[FirstVisibleCol, LastVisibleCol]` of the horizontal viewport. Each `DisplaySeg` is walked **once, never past `lastCol`** (so a multi-MB single-segment line costs O(window), not O(line length)); the intersecting byte slice becomes one `canvas.Text` at `ColX(depth, a-colBase)`, colored `pv.palette[seg.Role]` and sized `width*CharWidth × RowH` **directly** (no `MeasureText` — the grid is uniform). A `2*windowCols` hard cap bounds emitted runes, so every texture ≤ viewport width.
   - **Caveat — the O(window) bound is exact only for a byte==column grid line** (`LineIsByteGrid`: ASCII / no multibyte rune — the common case, including minified JSON; issue #4's fast path). On such a line, the window's start byte is found by arithmetic, so a reflow scrolled deep into a huge line walks O(window). A line containing **any multibyte rune** falls to the decode path, which still walks **once never past `lastCol`** but must decode from the segment start to *locate* `loByte`, so it pays **O(`FirstVisibleCol`)** — proportional to the horizontal scroll offset, not the visible window. This bounds only horizontal-scroll reflow cost on a single very long non-ASCII line; the row axis and all normal content are unaffected. A per-line rune-prefix index would restore O(window) there but adds model state, so it is deferred (#101). This is the same class of limitation as the CJK/proportional-font caveat in §11.
...
**Shipped: the scan is synchronous on the Fyne goroutine, not a worker.** With the single-pass byte→rune conversion (§7.2) and a hard `MaxMatches` cap, a full **plain** scan is O(total bytes) and stays under a frame — single-digit milliseconds (~5 ms, hardware-dependent) over the 7.5 MB / 440k-line fixture. Regex (`SearchRegex`) is heavier per line; a literal-prefix prefilter (`re.LiteralPrefix`) skips the RE2 engine on non-candidate lines, but a pattern with no literal head runs the engine on every line. Keystroke debouncing (`searchDebounced`, 150 ms, via `time.AfterFunc`+`fyne.Do`) keeps typing smooth in all cases. A `searchGen` counter gives last-query-wins: a debounce callback that fired before a newer keystroke / `ClearSearch` / `SetData` checks the generation and skips itself.
...
| R-10 | **O(K·L) byte→rune in search** — A2 Issue #5 | High (freeze) | **Resolved:** single forward pass per line (`colCursor`, §7.2); ~5 ms full **plain** scan of the 7.5 MB fixture (regex is heavier, with a literal-prefix prefilter), so the synchronous scan needs no chunking (§7.3). |
