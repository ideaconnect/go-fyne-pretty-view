...
func TestTapTogglesFold(t *testing.T) {
	...
	full := int(pv.doc.fold.TotalVisibleRows())

	// The top-level object's fold head is at row 0, depth 0.
	li := pv.doc.fold.lineAtRow(0)
	if pv.doc.Lines[li].Fold == NoNode {
	...
	if got := int(pv.doc.fold.TotalVisibleRows()); got != 1 {
		t.Fatalf("after collapsing root: visible=%d, want 1", got)
	}

	// Tap again: restores.
	pv.Tapped(&fyne.PointEvent{Position: fyne.NewPos(tx, 1)})
	if got := int(pv.doc.fold.TotalVisibleRows()); got != full {
