...
func TestSelectedTextMultiLine(t *testing.T) {
	...
	la := pv.doc.fold.lineAtRow(1)
	lb := pv.doc.fold.lineAtRow(2)
...
func TestCopyAfterCollapseIsWYSIWYG(t *testing.T) {
	...
	pv.doc.fold.fold(pv.doc, a) // row 1 now shows: "a": { 1 item },
...
func TestWordBounds(t *testing.T) {
	...
	li := pv.doc.fold.lineAtRow(1) // `"hello": "world"`
...
func TestSelectAllAndClear(t *testing.T) {
	...
	if n := strings.Count(got, "\n"); n != int(pv.doc.fold.TotalVisibleRows())-1 {
		t.Errorf("select-all newline count = %d, want %d", n, int(pv.doc.fold.TotalVisibleRows())-1)
	}
	pv.ClearSelection()
	if pv.SelectedText() != "" {
...
func TestDragSelectionIntegration(t *testing.T) {
	...
	li1 := pv.doc.fold.lineAtRow(1)
	li2 := pv.doc.fold.lineAtRow(2)
	x1, y1 := pv.doc.cellOrigin(pv.met, li1, 0)
	x2, y2 := pv.doc.cellOrigin(pv.met, li2, pv.doc.lineRuneLen(li2))
